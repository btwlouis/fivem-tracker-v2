...
async function getServers(
  ...
  try {
    ...
    console.log(
      `Fetched (${servers.length}) servers in ${performance.now() - perf}ms`
    );
...
export default async function Home({
  ...
  const { servers, totalCount, totalPages } = await getServers(
    locale,
    currentPage
  );
    ...
    <div className="container">
      ...
      <div>
        {servers.length === 0 ? (
          ...
          <Suspense fallback={<p className="text-white">Loading servers...</p>}>
            <DataTable
              columns={columns}
              data={servers}
